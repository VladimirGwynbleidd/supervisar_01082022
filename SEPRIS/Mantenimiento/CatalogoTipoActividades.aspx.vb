Imports Entities
Public Class CatalogoTipoActividades
...
#Region "Carga Datos"
    ...
    Private Sub CargarFiltros()
        ...
        ucFiltro1.AddFilter("ID", ucFiltro.AcceptedControls.TextBox, Nothing, "", "N_ID_TIPO_ACTIVIDAD", ucFiltro.DataValueType.IntegerType, False, False, False, False, False, Nothing, 3)
        ucFiltro1.AddFilter("Descripción", ucFiltro.AcceptedControls.TextBox, Nothing, "", "T_DSC_TIPO_ACTIVIDAD", ucFiltro.DataValueType.StringType, False, True)
        ...
        ucFiltro1.AddFilter("Requiere Autorización", ucFiltro.AcceptedControls.CheckBox, lstAutorizacion, "descripcion", "B_FLAG_REQUIERE_AUTORIZACION", ucFiltro.DataValueType.IntegerType, False, False, False, False, False, )

        ucFiltro1.LoadDDL("CatalogoTipoActividades.aspx")
...
#Region "Eventos Controles"
    ...
    Protected Sub btnAgregar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAgregar.Click
        ...
        lblTituloRegistro.Text = "Alta de Tipo de Actividad"

        Dim objTipoActividad As New TipoActividad()

        txtID.Text = objTipoActividad.ObtenerSiguienteIdentificador.ToString
    ...
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptar.Click
        ...
        Dim objTipoActividad As New TipoActividad(Convert.ToInt32(txtID.Text))

        If Not objTipoActividad.Existe Then
            Dim errores As New Entities.EtiquetaError(1088)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        Else
            Dim errores As New Entities.EtiquetaError(1089)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        End If
    ...
    Protected Sub btnModificar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnModificar.Click
        ...
        lblTituloRegistro.Text = "Modificación de Tipo de Actividad"

        If gvConsulta.SelectedIndex = -1 Then
            Dim errores As New Entities.EtiquetaError(1090)
            Mensaje = errores.Descripcion
        ...
        Dim objTipoActividad As New TipoActividad(CInt(gvConsulta.DataKeys(gvConsulta.SelectedIndex)("N_ID_TIPO_ACTIVIDAD").ToString()))

        If Not objTipoActividad.Vigente Then
            Dim errores As New Entities.EtiquetaError(1091)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        Else
            txtID.Text = objTipoActividad.Identificador
            txtTipoActividad.Text = objTipoActividad.Descripcion
            chkRequiereAutorizacion.Checked = objTipoActividad.RequiereAutorizacion
    ...
    Protected Sub btnCancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancelar.Click
        ...
        Dim errores As New Entities.EtiquetaError(1094)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Confirmacion", "MensajeConfirmacion();", True)
    ...
    Protected Sub btnEliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEliminar.Click
        ...
        Dim errores
        If gvConsulta.SelectedIndex = -1 Then
            errores = New Entities.EtiquetaError(1090)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        Dim entTipoActividad As New TipoActividad(CInt(gvConsulta.DataKeys(gvConsulta.SelectedIndex)("N_ID_TIPO_ACTIVIDAD").ToString()))

        If Not entTipoActividad.Vigente Then
            errores = New Entities.EtiquetaError(1091)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        errores = New Entities.EtiquetaError(1092)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
    ...
    Protected Sub btnAceptarM2B1A_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptarM2B1A.Click
        ...
        Select Case btnAceptarM2B1A.CommandArgument
            ...
            Case "btnAceptar"
                ...
                Dim objTipoActividad As New TipoActividad(Convert.ToInt32(txtID.Text))
                objTipoActividad.Descripcion = txtTipoActividad.Text
                objTipoActividad.RequiereAutorizacion = chkRequiereAutorizacion.Checked

                If Not objTipoActividad.Existe Then
                    objTipoActividad.Identificador = objTipoActividad.ObtenerSiguienteIdentificador()
                    objTipoActividad.Agregar()
                Else
                    objTipoActividad.Actualizar()
                End If
    ...
    Protected Sub btnConsulta_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnConsulta.Click
        ...
        lblTituloRegistro.Text = "Consulta de Tipo de Actividad"

        Dim index As Integer = Convert.ToInt32(Request("__EVENTARGUMENT"))

        Dim objTipoActividad As New TipoActividad(CInt(gvConsulta.DataKeys(index)("N_ID_TIPO_ACTIVIDAD").ToString()))

        txtID.Text = objTipoActividad.Identificador.ToString
        txtTipoActividad.Text = objTipoActividad.Descripcion
        chkRequiereAutorizacion.Checked = objTipoActividad.RequiereAutorizacion
    ...
    Protected Sub btnExportaExcel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExportaExcel.Click
        ...
        utl.ExportaGrid(dt, gvConsulta, "Catálogo de Tipo de Actividades", referencias)
    End Sub
...
#Region "Validaciones"
    ...
    Private Sub cvTipoActividad_ServerValidate(ByVal source As Object, ByVal args As System.Web.UI.WebControls.ServerValidateEventArgs) Handles cvTipoActividad.ServerValidate
        If txtTipoActividad.Text.Trim() = String.Empty Then
            Dim errores As New Entities.EtiquetaError(1093)
