...
Public Class CatalogoOpcionesEvaluacion
...
#Region "Carga Datos"
    ...
    Private Sub CargarFiltros()
        ...
        ucFiltro1.AddFilter("ID", ucFiltro.AcceptedControls.TextBox, Nothing, "", "N_ID_ENCUESTA_OPCION", ucFiltro.DataValueType.IntegerType, False, False, False, False, False, Nothing, 3)
        ucFiltro1.AddFilter("Descripción", ucFiltro.AcceptedControls.TextBox, Nothing, "", "T_DSC_OPCION", ucFiltro.DataValueType.StringType, False, True)
        ucFiltro1.LoadDDL("CatalogoOpcionesEvaluacion.aspx")
    ...
    Private Sub CargarCatalogo()
        ...
        Dim objOpcion As New OpcionesEvaluacion()
        Dim dv As DataView = objOpcion.ObtenerTodos()
...
#Region "Eventos Controles"
    ...
    Protected Sub btnAgregar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAgregar.Click
        ...
        lblTituloRegistro.Text = "Alta de Opción a Evaluar"

        Dim objOpcion As New OpcionesEvaluacion()

        txtId.Text = objOpcion.ObtenerSiguienteIdentificador.ToString
    ...
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptar.Click
        ...
        Dim objOpcion As New OpcionesEvaluacion(Convert.ToInt32(txtId.Text))

        If Not objOpcion.Existe Then
            Dim errores As New Entities.EtiquetaError(1126)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        Else
            Dim errores As New Entities.EtiquetaError(1127)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        End If
    ...
    Protected Sub btnModificar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnModificar.Click
        ...
        lblTituloRegistro.Text = "Modificación de Opción a Evaluar"

        If gvConsulta.SelectedIndex = -1 Then
            Dim errores As New Entities.EtiquetaError(1130)
            Mensaje = errores.Descripcion
        ...
        Dim objOpcion As New OpcionesEvaluacion(CInt(gvConsulta.DataKeys(gvConsulta.SelectedIndex)("N_ID_ENCUESTA_OPCION").ToString()))

        If Not objOpcion.Vigente Then
            Dim errores As New Entities.EtiquetaError(1131)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        Else
            txtId.Text = objOpcion.Identificador
            txtOpcion.Text = objOpcion.Descripcion
    ...
    Protected Sub btnCancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancelar.Click
        ...
        Dim errores As New Entities.EtiquetaError(1128)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Confirmacion", "MensajeConfirmacion();", True)
    ...
    Protected Sub btnEliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEliminar.Click
        ...
        Dim errores
        If gvConsulta.SelectedIndex = -1 Then
            errores = New Entities.EtiquetaError(1130)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        If Not objOpcion.Vigente Then
            errores = New Entities.EtiquetaError(1131)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        errores = New Entities.EtiquetaError(1129)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
    ...
    Protected Sub btnAceptarM2B1A_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptarM2B1A.Click
        ...
        Select Case btnAceptarM2B1A.CommandArgument
            ...
            Case "btnAceptar"
                ...
                Dim objOpcion As New OpcionesEvaluacion(Convert.ToInt32(txtId.Text))
                objOpcion.Descripcion = txtOpcion.Text()

                If Not objOpcion.Existe Then
                    objOpcion.Identificador = objOpcion.ObtenerSiguienteIdentificador()
                    objOpcion.Agregar()
                Else
                    objOpcion.Actualizar()
                End If
    ...
    Protected Sub btnConsulta_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnConsulta.Click
        ...
        lblTituloRegistro.Text = "Consulta de Opcion de Evaluación"

        Dim index As Integer = Convert.ToInt32(Request("__EVENTARGUMENT"))

        Dim objOpcion As New OpcionesEvaluacion(CInt(gvConsulta.DataKeys(index)("N_ID_ENCUESTA_OPCION").ToString()))

        txtId.Text = objOpcion.Identificador.ToString
        txtOpcion.Text = objOpcion.Descripcion
    ...
    Protected Sub btnExportaExcel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExportaExcel.Click
        ...
        utl.ExportaGrid(dt, gvConsulta, "Opciones de Evaluación", referencias)
    End Sub
...
#Region "Validaciones"
    ...
    Private Sub cvTipoOpcion_ServerValidate(ByVal source As Object, ByVal args As System.Web.UI.WebControls.ServerValidateEventArgs) Handles cvOpcion.ServerValidate
        If txtOpcion.Text.Trim() = String.Empty Then
            Dim errores As New Entities.EtiquetaError(1132)
