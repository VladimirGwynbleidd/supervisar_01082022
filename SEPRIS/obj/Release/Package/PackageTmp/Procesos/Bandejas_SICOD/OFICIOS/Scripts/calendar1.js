...
function cal_prs_date1 (str_date) {
    ...
    if (!arr_date[0]) return cal_error ("Formato de Fecha Invalido: '" + str_date + "'.\nValor Dia de Mes No Encontrado.");
    if (!RE_NUM.exec(arr_date[2])) return cal_error ("Valor de Dia de Mes Invalido: '" + arr_date[0] + "'.\nValores Permitidos Numericos.");
    if (!arr_date[1]) return cal_error ("Formato de Fecha Invalido: '" + str_date + "'.\nValor de Mes No Encontrado.");
    if (!RE_NUM.exec(arr_date[1])) return cal_error ("Valor de Mes Invalido: '" + arr_date[1] + "'.\nValores Permitidos Usar Enteros.");
    if (!arr_date[2]) return cal_error ("Formato de Fecha Invalido: '" + str_date + "'.\nValor de Año No Encontrado.");
    if (!RE_NUM.exec(arr_date[0])) return cal_error ("Valor de Año Invalido: '" + arr_date[2] + "'.\nValores Permitidos Usar Enteros.");
