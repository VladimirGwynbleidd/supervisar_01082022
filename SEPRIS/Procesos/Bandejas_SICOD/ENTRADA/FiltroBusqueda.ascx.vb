        ...
        Select Case actualizaNombreFiltro
            Case "Tipo de Documento"
                ...
                If pnlTipoDeDocto.Visible Then
                    ...
                    Try
                        Con.ConsultaAdapter("SELECT ID_T_DOC,DSC_T_DOC FROM " & Clases.Conexion.Owner & "BDA_C_T_DOC WHERE VIG_FLAG= 1").Fill(dt)
                        ddlTipoDocto.DataSource = dt
                        ddlTipoDocto.DataTextField = "DSC_T_DOC"
                        ddlTipoDocto.DataValueField = "ID_T_DOC"
                ...
                If pnlArea.Visible Then
                    ...
                    Try
                        ...
                        If Session("Perfil").ToString() = System.Web.Configuration.WebConfigurationManager.AppSettings("UsuarioAdmon") Then
                            Con.ConsultaAdapter("SELECT ID_UNIDAD_ADM, DSC_UNIDAD_ADM FROM " & Clases.Conexion.Owner & "BDA_C_UNIDAD_ADM WHERE VIG_FLAG= 1 AND ID_T_UNIDAD_ADM = 2 ORDER BY 2").Fill(dt)
                        Else
                            Con.ConsultaAdapter("SELECT ID_UNIDAD_ADM, DSC_UNIDAD_ADM FROM " & Clases.Conexion.Owner & "BDA_C_UNIDAD_ADM WHERE VIG_FLAG= 1 AND ID_T_UNIDAD_ADM = 2 AND ID_UNIDAD_ADM = " & Session("UnidadAdm").ToString() & " ORDER BY 2").Fill(dt)
                        End If
            ...
            Case "Responsable"
                ...
                If pnlNombre.Visible Then
                    ...
                    Try
                        ...
                        Con.ConsultaAdapter(" SELECT U.USUARIO, NOMBRE + ' ' + ISNULL(APELLIDOS, '') NOMBRE FROM " & Clases.Conexion.Owner & "BDS_USUARIO U JOIN " & Clases.Conexion.Owner & "BDA_R_USUARIO_UNIDAD_ADM A ON U.USUARIO = A.USUARIO WHERE U.VIG_FLAG = 1 AND A.VIG_FLAG = 1 AND A.ID_T_UNIDAD_ADM = 2  ORDER BY 2").Fill(dt)

                        If dt.Rows.Count > 0 Then
                            txtNombre.DataSource = dt
                            txtNombre.DataTextField = "NOMBRE"
                            txtNombre.DataValueField = "USUARIO"
                            txtNombre.DataBind()
                            txtNombre.Items.Insert(0, "-Seleccione Uno-")
                        Else
                            txtNombre.Items.Clear()
                            txtNombre.Items.Insert(0, "-No se encontraron Responsables-")
                            txtNombre.DataBind()
                        End If
        ...
        If pnlReferencia.Visible Then
            Try
                If TxtRefere.Text <> "" Then
                    If Not panelVisibles.ToString().Contains("Referencia") Then
                        panelVisibles = panelVisibles & "Referencia" & "@"
                        Session("TxtRefere") = TxtRefere.Text
                    End If
                ...
                If TxtFecRecIni.Text <> "" And TxtFecRecFin.Text <> "" Then
                    ...
                    Else
                        ...
                        If (Not fechasErroneas And d2 >= d1 And d1 < Date.Now) Then
                            ...
                            query &= " AND FECH_RECEPCION BETWEEN " & "'" & fechaD1 & "'"
        ...
        Try
            ...
            If pnlTipoDeDocto.Visible Then
                If ddlTipoDocto.SelectedIndex > 0 Then
                    ...
                    End If
                    query &= " AND ID_T_DOC = '" & ddlTipoDocto.SelectedValue & "'"
                Else
            ...
            If pnlEstatus.Visible Then
                If rdbRec.Checked Then
                    ...
                    If Not panelVisibles.ToString().Contains("Recibido") Then
                        panelVisibles = panelVisibles & "Recibido" & "@"
                        Session("rdbRec") = rdbRec.Checked
                    End If
                ...
                ElseIf rdbNRec.Checked Then
                    ...
                    If Not panelVisibles.ToString().Contains("Recibido") Then
                        panelVisibles = panelVisibles & "Recibido" & "@"
                        Session("rdbNRec") = rdbNRec.Checked
                    End If
                Else
            ...
            If pnlFolio.Visible Then
                Try
                    If txtFolio.Text <> "" Then
                        ...
                        If Not panelVisibles.ToString().Contains("Folio") Then
                            panelVisibles = panelVisibles & "Folio" & "@"
                            Session("txtFolio") = txtFolio.Text
                        End If
            ...
            '            If Not panelVisibles.ToString().Contains("Remitente") Then
            '                panelVisibles = panelVisibles & "Remitente" & "@"
            '                Session("txtRemitente") = txtRemitente.Text
            '            End If
            ...
            '            If Not panelVisibles.ToString().Contains("Responsable") Then
            '                panelVisibles = panelVisibles & "Responsable" & "@"
            '                Session("txtNombre") = txtNombre.Text
            '            End If

            '            query &= " AND USUARIO_RESPONSABLE = '" & txtNombre.Text.ToString().Trim() & "'"
            '        Else
            ...
            If pnlFechaRegistro.Visible Then
                Try
                    ...
                    Dim fecharegistro = Date.ParseExact(txtFechaRegistro.Text.Trim(), "dd/MM/yyyy", Globalization.CultureInfo.InvariantCulture).ToString("yyyyMMdd")

                    If txtFechaRegistro.Text <> "" Then

                        If Not panelVisibles.ToString().Contains("Fecha de Registro") Then
                            panelVisibles = panelVisibles & "Fecha de Registro" & "@"
                            Session("txtFechaRegistro") = txtFechaRegistro.Text
                        End If

                        query &= "  AND CONVERT (DATE, FECH_REGISTRO) = '" & fecharegistro & "'"
            ...
            '            query &= " AND FECHA_LIMITE = '" & fechaLimite & "'"
            ...
            '            If Not panelVisibles.ToString().Contains("Estatus") Then
            '                panelVisibles = panelVisibles & "Estatus" & "@"
            '                Session("ddlStatus") = ddlStatus.Text
            '            End If
            '            Session("AtendidaStatus") = ddlStatus.SelectedValue

            '            query &= " AND ATENDIDA = '" & ddlStatus.SelectedValue & "'"
            ...
            '            If Not panelVisibles.ToString().Contains("Proviene SIE") Then
            '                panelVisibles = panelVisibles & "Proviene SIE" & "@"
            '                Session("rbtnSIE_SI") = rbtnSIE_SI.Checked
            '            End If
            ...
            '            If Not panelVisibles.ToString().Contains("Proviene SIE") Then
            '                panelVisibles = panelVisibles & "Proviene SIE" & "@"
            '                Session("rbtnSIE_NO") = rbtnSIE_NO.Checked
            '            End If
            '        Else
            ...
            If Session("SoloMiosV") IsNot Nothing Then
                query &= " AND USUARIO='" & Session("Usuario") & "'"

            End If


            'If Me.CheckBox1.Checked = True Then
            '    Session("SoloMios") = "SoloMios"

            'Else
            '    Session.Remove("SoloMios")
            'End If
