...
Public Class CatalogoAspectosEvaluar
...
#Region "Carga Datos"
    ...
    Private Sub CargarFiltros()
        ...
        ucFiltro1.AddFilter("ID", ucFiltro.AcceptedControls.TextBox, Nothing, "", "N_ID_ENCUESTA_ASPECTO_EVALUAR", ucFiltro.DataValueType.IntegerType, False, False, False, False, False, Nothing, 3)
        ucFiltro1.AddFilter("Descripción", ucFiltro.AcceptedControls.TextBox, Nothing, "", "T_DSC_ASPECTO_EVALUAR", ucFiltro.DataValueType.StringType, False, True)
        ucFiltro1.LoadDDL("CatalogoAspectosEvaluar.aspx")
    ...
    Private Sub CargarCatalogo()
        ...
        Dim objAspecto As New Aspecto
        Dim dv As DataView = objAspecto.ObtenerTodos()
...
#Region "Eventos Controles"
    ...
    Protected Sub btnAgregar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAgregar.Click
        ...
        lblTituloRegistro.Text = "Alta de Aspecto a Evaluar"

        Dim objAspecto As New Aspecto()

        txtId.Text = objAspecto.ObtenerSiguienteIdentificador.ToString
    ...
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptar.Click
        ...
        Dim objAspecto As New Aspecto(Convert.ToInt32(txtId.Text))

        If Not objAspecto.Existe Then
            Dim errores As New Entities.EtiquetaError(1119)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        Else
            Dim errores As New Entities.EtiquetaError(1120)
            Mensaje = errores.Descripcion
            imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        End If
    ...
    Protected Sub btnModificar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnModificar.Click
        ...
        lblTituloRegistro.Text = "Modificación de Aspecto a Evaluar"

        If gvConsulta.SelectedIndex = -1 Then
            Dim errores As New Entities.EtiquetaError(1123)
            Mensaje = errores.Descripcion
        ...
        Dim objAspecto As New Aspecto(CInt(gvConsulta.DataKeys(gvConsulta.SelectedIndex)("N_ID_ENCUESTA_ASPECTO_EVALUAR").ToString()))

        If Not objAspecto.Vigente Then
            Dim errores As New Entities.EtiquetaError(1124)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        Else
            txtId.Text = objAspecto.Identificador
            txtAspecto.Text = objAspecto.Descripcion
    ...
    Protected Sub btnCancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancelar.Click
        ...
        Dim errores As New Entities.EtiquetaError(1121)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Confirmacion", "MensajeConfirmacion();", True)
    ...
    Protected Sub btnEliminar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEliminar.Click
        ...
        Dim errores
        If gvConsulta.SelectedIndex = -1 Then
            errores = New Entities.EtiquetaError(1123)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        If Not objAspecto.Vigente Then
            errores = New Entities.EtiquetaError(1124)
            Mensaje = errores.Descripcion
            imgUnBotonNoAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Mostraremos", "AquiMuestroMensaje();", True)
            Exit Sub
        ...
        errores = New Entities.EtiquetaError(1122)
        Mensaje = errores.Descripcion
        imgDosBotonesUnaAccion.ImageUrl = Entities.Imagen.RutaCarpeta & errores.Imagen.Ruta
    ...
    Protected Sub btnAceptarM2B1A_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAceptarM2B1A.Click
        ...
        Select Case btnAceptarM2B1A.CommandArgument
            ...
            Case "btnAceptar"
                ...
                Dim objAspecto As New Aspecto(Convert.ToInt32(txtId.Text))
                objAspecto.Descripcion = txtAspecto.Text()

                If Not objAspecto.Existe Then
                    objAspecto.Identificador = objAspecto.ObtenerSiguienteIdentificador()
                    objAspecto.Agregar()
                Else
                    objAspecto.Actualizar()
                End If
    ...
    Protected Sub btnConsulta_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnConsulta.Click
        ...
        lblTituloRegistro.Text = "Consulta de Aspecto a Evaluar"

        Dim index As Integer = Convert.ToInt32(Request("__EVENTARGUMENT"))

        Dim objAspecto As New Aspecto(CInt(gvConsulta.DataKeys(index)("N_ID_ENCUESTA_ASPECTO_EVALUAR").ToString()))

        txtId.Text = objAspecto.Identificador.ToString
        txtAspecto.Text = objAspecto.Descripcion
    ...
    Protected Sub btnExportaExcel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExportaExcel.Click
        ...
        utl.ExportaGrid(dt, gvConsulta, "Catálogo de Aspectos a Evaluar", referencias)
    End Sub
...
#Region "Validaciones"
    ...
    Private Sub cvTipoAspecto_ServerValidate(ByVal source As Object, ByVal args As System.Web.UI.WebControls.ServerValidateEventArgs) Handles cvAspecto.ServerValidate
        If txtAspecto.Text.Trim() = String.Empty Then
            Dim errores As New Entities.EtiquetaError(1125)
