...
#End Region
    ...
    Protected Sub imgNotificar3_Click(sender As Object, e As ImageClickEventArgs) Handles imgNotificar3.Click
        If Not IsNothing(Session("DETALLE_VISITA_V17")) And Not IsNothing(Entities.Usuario.SessionID) Then
            ...
            'If visita.IdPasoActual <> 5 Or visita.IdPasoActual <> 12 Then
                                    ...
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_INS
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            If Not IsNothing(Session("DETALLE_VISITA_V17")) And Not IsNothing(Entities.Usuario.SessionID) Then
                                                ...
                                                'If Session("RESPONDIO_AFORE_P14") = Nothing Then
                                ...
                                Select Case Usuario.IdentificadorPerfilActual
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_INS ''Atiende observaciones VJ
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            ...
                                            objNegVisita.PasoGenerericEstatusPasoNotificarReactivar(True, Constantes.EstatusPaso.AjustesEnviados,
                                                                                                    False, -1,
                                                                                                    False, visita.IdPasoActual, Constantes.EstatusPaso.AjustesEnviados,
                                                                                                    True, Constantes.CORREO_ATIENDE_AJUSTES,
                                                                                                    True, False, False,,,,,,,,,,
                                ...
                                Select Case Usuario.IdentificadorPerfilActual
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_INS
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            ...
                                            Dim query As String = "SELECT * FROM BDS_D_VS_DOCUMENTO_PASO WHERE I_ID_VISITA=" & visita.IdVisitaGenerado & " AND I_ID_PASO=4 AND T_FOLIO_SICOD IS NOT NULL"
                                            Dim dtDictamen As DataTable = conexionC.ConsultarDT(query)
                                            conexionC.CerrarConexion()
                                            If dtDictamen.Rows.Count > 0 Then
                                                For Each documento As DataRow In dtDictamen.Rows
                                                    If Not ConexionSICOD.FolioFinalizado(documento("T_FOLIO_SICOD")) Then
                                                        Bandera = True
                                                        Exit For
                                                    End If
                                                Next
                                                'Verificamos si hubo algún registro en la Finalizado
                                                If (Bandera) Then
                                                    Dim errores As New Entities.EtiquetaError(2140)
                                                    Mensaje = errores.Descripcion
                                ...
                                Select Case Usuario.IdentificadorPerfilActual
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_INS, Constantes.PERFIL_SUP
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            Dim Bandera As Boolean = False
                                            Dim conexionC As New Conexion.SQLServer()
                                            Dim query As String = "SELECT * FROM BDS_D_VS_DOCUMENTO_PASO WHERE I_ID_VISITA=" & visita.IdVisitaGenerado & " AND I_ID_PASO=11 AND T_FOLIO_SICOD IS NOT NULL"
                                            Dim dtDictamen As DataTable = conexionC.ConsultarDT(query)
                                            conexionC.CerrarConexion()
                                            If dtDictamen.Rows.Count > 0 Then
                                                For Each documento As DataRow In dtDictamen.Rows
                                ...
                                Select Case Usuario.IdentificadorPerfilActual
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_INS
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            Dim Bandera As Boolean = False
                                            Dim conexionC As New Conexion.SQLServer()
                                            Dim query As String = "SELECT * FROM BDS_D_VS_DOCUMENTO_PASO WHERE I_ID_VISITA=" & visita.IdVisitaGenerado & " AND I_ID_PASO=16 AND T_FOLIO_SICOD IS NOT NULL"
                                            Dim dtDictamen As DataTable = conexionC.ConsultarDT(query)
                                            conexionC.CerrarConexion()
                                            If dtDictamen.Rows.Count > 0 Then
                                                For Each documento As DataRow In dtDictamen.Rows
                            ...
                            Case Constantes.EstatusPaso.EnRevisionEspera ''Paso 19 guardar comentarios
                                Select Case Usuario.IdentificadorPerfilActual
                                    Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_INS
                                        If EsAreaOperativa(Usuario.IdArea) Then
                                            Dim irregularidadesRegistradas As DataTable = Entities.IrregularidadVisita.ObtenerTodas(visita.IdVisitaGenerado)
                                            Dim Bandera17 As Boolean = False
                                            If irregularidadesRegistradas.Rows.Count > 0 Then
                                                Dim conexionC As New Conexion.SQLServer()
                                                Dim query As String = "SELECT * FROM BDS_D_VS_DOCUMENTO_PASO WHERE I_ID_VISITA=" & visita.IdVisitaGenerado & " AND N_ID_DOCUMENTO=" & 37 & " AND T_FOLIO_SICOD IS NOT NULL"
                                                Dim dtDictamen As DataTable = conexionC.ConsultarDT(query)
                                                conexionC.CerrarConexion()
                                                If dtDictamen.Rows.Count > 0 Then
                                                    For Each documento As DataRow In dtDictamen.Rows
...
#End Region
    ...
    Protected Sub btnConfFechaReunionVjp16_Click(sender As Object, e As EventArgs)
        ...
        If Convert.ToDateTime(lsFecha) > DateTime.Now.Date Then
            ...
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "Confirmacion", "ConfirmarFechaReunPreciJuridicoP16();", True)
            Exit Sub
        End If
        ...
        If AccesoBD.ActualizaFechaInicioVisita(objVisita.IdVisitaGenerado, lsFecha, Constantes.TipoFecha.FechaReunionVjp16Confirmacion, ppObjVisita.SubVisitasSeleccionadas) Then
    ...
    Protected Sub btnFechaRetroalimentacion_Click(sender As Object, e As EventArgs)
        ...
        If objVisita.IdPasoActual = 8 Then
            objNegVisita.PasoGenerericEstatusPasoNotificarReactivar(True, ppObjVisita.IdEstatusActual,
                                                                 True, Constantes.EstatusPaso.Aprobado,
                                                                 True, 9, Constantes.EstatusPaso.EnRevisionEspera,''REVISAR DETALLE DEL PASO 8
                                                                 True, Constantes.CORREO_ID_NOTIFICA_APRUEBA_ACTA_CIRCUNSTANCIADA,,,,,,,,, CDate(lsFecha),,,, Constantes.MovimientoPaso.ApruebaActaCircunstanciada)
...
'Se mofica caso de 20 al 17
    ...
    Public Shared Function ValidacionSISAN(Folio As Integer, FolioVisita As String, FechaInicioActoSup As String, FechaFinActoSup As String, Usuario As String, IdEntidad As Integer, IdTipoEntidad As Integer) As String
        ...
        Dim querySubvisitas As String = "SELECT * FROM BDS_D_VS_VISITA WHERE I_ID_VISITA_PADRE_SB = " & Folio & " and I_ID_PASO_ACTUAL=17"
            ...
            If Not resultadoRegistro.isError Then
                ...
                For Each drSubvisita As DataRow In dtSubvisitas.Rows
                    ...
                    If Not resultadoRegistroSubVisita.isError Then
                        ...
                        MensajeResult = MensajeResult + "|" + folioSubvisitaSISAN
    ...
    End Sub
    Protected Sub btnCancelarFechaPaso7_Click(sender As Object, e As EventArgs)
        lblFechaGeneral.Text = String.Empty
        lblFechaCampo.Text = String.Empty
        lblErrorFecIni.Text = String.Empty
        txtFechRevHallazgos.Text = String.Empty
        lblPresentaHallazgos.Text = String.Empty
    End Sub
    Protected Sub btnCambioNormativaLimpiar_Click(sender As Object, e As EventArgs)
        txbFechFinVisita.Text = String.Empty
        lblMensajeFechaFinVisitaObligatorio.Text = String.Empty
    End Sub
